...
from utils.bnm_utils import configure_model, collect_params, BNM
from utils.bnm_utils import setup_bnm, setup_optimizer
# ----------------------------------
...
########################################################################
parser.add_argument('--method', default='bnm', choices=['bnm'])
########################################################################
...
print("Test-time adaptation: BNM")
bnm_model = setup_bnm(net, args)
bnm_model.eval()

print('Error (%)\t\ttest\t\tbnm')
...
if args.method == 'bnm':
    prefix = os.path.join(args.outf, args.corruption + '_bnm')
else:
